...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)  #(sents, sent_len, word_len)(int)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        alpha_t_m = torch.unsqueeze(alpha_t, dim=1)
        a_t = torch.squeeze(torch.bmm(alpha_t_m, enc_hiddens), dim=1)
        #U_t = torch.cat((dec_hidden, a_t), dim=1)
        U_t = torch.cat((a_t, dec_hidden), dim=1)
        V_t = self.combined_output_projection(U_t)
        O_t = self.dropout(torch.tanh(V_t))
        ### END YOUR CODE FROM ASSIGNMENT 4
        #(b, 2h) = (b, 1, len) * (b, len, 2*h)
